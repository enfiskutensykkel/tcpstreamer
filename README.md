...
much like creating _plug-in_s for a program.
...
```
#include "streamer.h" // we need this in order to get the typedef and STREAMER macro
#include <unistd.h>
#include <stdarg.h>

static int run = 1;

ssize_t example_streamer(int socket_desc, ...); // streamer entry point
void example_signal_handler(streamer_t running_instance); // streamer signal handler
...
void example_signal_handler(streamer_t running_instance)
{
	if (running_instance == &example_streamer) {
		run = 0;
	}
}
```
